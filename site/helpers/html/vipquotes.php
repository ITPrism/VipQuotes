...
abstract class JHtmlVipQuotes
{
     ...
     * @return string
     */
    public static function authorImage($item, $params)
    {
        $html = array();
        ...
        if (!$item->thumb) {
            ...
            if (!$params->get("images_display_default", 1)) {
                ...
                $html[] = '<a href="' . JRoute::_(VipQuotesHelperRoute::getAuthorRoute($item->slug)) . '">';
    ...
    public static function information($item, $categoryTitle, $displayPublisher, $socialProfile = "")
    {
        if (is_array($item)) {
        ...
        // Display category
        $category = '<a href="' . JRoute::_(VipQuotesHelperRoute::getCategoryRoute($item->catid)) . '">' . $categoryTitle . '</a>';
